...
export default function ContentPPVGate({
  videoId,
  livestreamId,
  title,
  price = 999,
  ...
  children,
  onRequireSignIn,
  purchaseLabel = "Unlock Access",
  valueProp = "One-time purchase for this replay.",
  footerText = "One-time purchase. Replay stays unlocked for future viewing.",
  ...
  const entitlementArgs = useMemo(() => {
    if (!isConvexAuthenticated || !userId || (!videoId && !livestreamId)) {
      return "skip";
    }
  ...
  }, [isConvexAuthenticated, livestreamId, userId, videoId]);

  const hasEntitlement = useQuery(api.entitlements.hasBundledEntitlement, entitlementArgs);
  ...
  const handlePurchase = useCallback(async () => {
    ...
    try {
      const callbackPath = redirectPath || (videoId ? `watch/${videoId}` : "ppv-callback");
      const redirectUrl = AuthSession.makeRedirectUri({ path: callbackPath });
  ...
  }, [isSignedIn, livestreamId, onRequireSignIn, redirectPath, sessionId, videoId]);

  const handleOpenWebFallback = useCallback(async () => {
    try {
      await WebBrowser.openBrowserAsync(getWebUrl(webFallbackPath));
    } catch (error) {
      console.error("[ContentPPVGate] failed to open web fallback:", error);
    }
  }, [webFallbackPath]);
      ...
      >
        <View style={{ position: "absolute", top: 12, left: 12 }}>
          ...
          >
            ...
            <Text style={{ color: "#000", fontWeight: "800", fontSize: 10, letterSpacing: 1 }}>
              {badgeLabel}
            </Text>
          </View>
        </View>
        ...
        <Text style={{ color: "#999", fontSize: 13, marginBottom: 24, textAlign: "center" }}>
          {valueProp}
        </Text>
